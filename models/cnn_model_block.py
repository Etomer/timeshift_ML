...
class model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.thinker = nn.Sequential(
            ...
            nn.Linear(config["scale_factor"]*config["cnn_output_size_at_factor_1"],1000),
            nn.GELU(),
            Block(1000,config["dropout"]),
            Block(1000,config["dropout"]),
        ...
        self.cnn = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv1d(48*config["scale_factor"],48*config["scale_factor"], 50,stride=5),
            nn.GELU(),
            nn.Conv1d(48*config["scale_factor"],48*config["scale_factor"], 30,stride=5),
            nn.GELU(),
            nn.Flatten(),
        )
