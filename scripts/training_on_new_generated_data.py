...
with wandb.init(config=config) if use_wandb else nullcontext() as run:
    ...
    shutil.copyfile(
        os.path.join(model_folder, config["model"] + ".py"),
        os.path.join(run_folder, config["model"] + ".py"),
    )
    ...
    with h5py.File(data_folder + config["dataset"], "r") as f:
        ...
        class custom_dataset(Dataset):
            ...
            def __getitem__(self, idx):
                mics_idx = torch.randperm(X.shape[1])[
                    : config["mics_per_batch"]
                ].sort()[
                    0
                ]  # sorting because hdf5 requires it
                return (
                    self.X[idx + self.idx_min, mics_idx],
                    self.y[idx + self.idx_min, mics_idx],
                )
        ...
        train_dl = DataLoader(
            dataset, batch_size=config["rooms_per_batch"], shuffle=True
        )
        test_dl = DataLoader(
            dataset_test, batch_size=config["rooms_per_batch"], shuffle=False
        )
        ...
        def train(dataloader, model, loss_fn, optimizer, scheduler, warm_up=False):
            ...
            for batch, (X, y) in enumerate(dataloader):
                ...
                if (
                    warm_up
                    and batch
                    % (1 + (len(dataloader) // config["warmup_steps_per_epoch"]))
                    == 0
                ):
                    scheduler.step()
        ...
        if config["optimizer"] == "adamw":
            optimizer = torch.optim.AdamW(
                model.parameters(), lr=config["lr"] / 2**schedule_steps
            )
        ...
        for t in range(config["epochs"]):
            ...
            if t == config["warmup_epochs"]:
                print(
                    f'WarmUp done! now using lr = {optimizer.param_groups[0]["lr"]:.2}'
                )
            ...
            train(
                ...
                optimizer,
                scheduler,
                warm_up=(t < config["warmup_epochs"]),
            )
            ...
            if t % 100 == 0:
                if use_wandb:
                    ...
                    torch.save(
                        model,
                        os.path.join(
                            run_folder, "checkpoints", run.name + "_" + str(t) + ".pth"
                        ),
                    )
                else:
                    ...
                    torch.save(
                        model,
                        os.path.join(
                            run_folder, "checkpoints", "not_logged_" + str(t) + ".pth"
                        ),
                    )
