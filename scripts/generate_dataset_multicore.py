    ...
    with h5py.File(path, "w") as hdf5_file:
        ...
        for ii in range(n_batches):
            ...
            start = np.random.randint(
                len(signal)
                - sample_length * batch_simulate_at_same_locations
                - extra_bit
            )
            # simulate longer sound and then cut to the relevant piece
            signal = signal[
                start : start
                + batch_simulate_at_same_locations * sample_length
                + extra_bit
            ]
...
if __name__ == "__main__":
    ...
    with h5py.File(dataset_path + ".hdf5", "w") as hdf5_file:
        X = hdf5_file.create_dataset(
            "input", (n_core * dataset_size, 2, max_freq_component_to_use, 2), dtype="f"
        )
