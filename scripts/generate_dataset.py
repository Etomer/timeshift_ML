...
with h5py.File(
    "./datasets/generated_dataset/generated_dataset_valer.hdf5", "w"
) as hdf5_file:
    X = hdf5_file.create_dataset(
        "input", (dataset_size, 2, max_freq_component_to_use, 2), dtype="f"
    )
    ...
    for ii in range(n_batches):
        ...
        start = np.random.randint(
            len(signal) - sample_length * batch_simulate_at_same_locations - extra_bit
        )
        # simulate longer sound and then cut to the relevant piece
        signal = signal[
            start : start + batch_simulate_at_same_locations * sample_length + extra_bit
        ]
        ...
        gt_tdoa = (
            (
                np.linalg.norm(sender_position - receiver1_position)
                - np.linalg.norm(sender_position - receiver2_position)
            )
            * fs
            / speed_of_sound
        )
        ...
        for i in range(batch_simulate_at_same_locations):
            ...
            temp = sp.fft.fft(
                (
                    room.mic_array.signals[
                        0,
                        extra_bit
                        + (i) * sample_length : extra_bit
                        + (i + 1) * sample_length,
                    ]
                )
            )
            X[new_example_index, 0, :, 0] = torch.tensor(
                np.real(temp[:max_freq_component_to_use])
            )
            X[new_example_index, 1, :, 0] = torch.tensor(
                np.imag(temp[:max_freq_component_to_use])
            )
            temp = sp.fft.fft(
                (
                    room.mic_array.signals[
            ...
            X[new_example_index, 0, :, 1] = torch.tensor(
                np.real(temp[:max_freq_component_to_use])
            )
            X[new_example_index, 1, :, 1] = torch.tensor(
                np.imag(temp[:max_freq_component_to_use])
            )
